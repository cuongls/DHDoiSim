...
{
    public class UploadCamKetKhongCCController : Controller
    {
        DHDoiSimEntities db = new DHDoiSimEntities();
        // GET: UploadCamKetKhongCC
        ...
        public ActionResult UploadFile(HttpPostedFileBase file, string idsp)
        {
            try
            {
                if (file.ContentLength > 0)
                {
                    ...
                    update.Image_CamKetKhongCC = _FileName;
                    db.SaveChanges();
                }

                ViewBag.Message = "File Uploaded Successfully!idsp = " + idsp.ToString();
                //return View();
                return RedirectToAction("../NhanKhoaPhieu/EditKhoaPhieu/" + idsp);
