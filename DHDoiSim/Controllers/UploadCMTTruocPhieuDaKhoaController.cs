...
{
    public class UploadCMTTruocPhieuDaKhoaController : Controller
    {
        //User user = new User();
        DHDoiSimEntities db = new DHDoiSimEntities();
        // GET: UploadCMTTruocPhieuDaKhoa
        ...
        public ActionResult UploadFile(HttpPostedFileBase file, string idsp)
        {
            try
            {
                if (file.ContentLength > 0)
                {
                    ...
                    update.Image_CMT_Truoc = _FileName;
                    db.SaveChanges();
                }

                ViewBag.Message = "File Uploaded Successfully!idsp = " + idsp.ToString();
                //return View();
                return RedirectToAction("../NhanKhoaPhieu/UpdatPhieuDaKhoa/" + idsp);
